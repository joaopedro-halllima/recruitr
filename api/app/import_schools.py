...
def _normalize_row(raw_row: dict[str, str]) -> dict | None:
   ...
   website = _pick(row, "webaddr", "web_addr", "website", "url")
   sector = _pick(row, "control", "sector")
   level = _pick(row, "iclevel", "institution_level", "level")
...
def import_csv(csv_path: Path, batch_size: int = 1000) -> tuple[int, int]:
   ...
   upsert_sql = text(
       ...
       INSERT INTO public.schools
         (unitid, name, city, state, zip, website, sector, level, conference, logo_url)
       VALUES
         (:unitid, :name, :city, :state, :zip, :website, :sector, :level, :conference, :logo_url)
       ON CONFLICT (unitid)
       DO UPDATE SET
